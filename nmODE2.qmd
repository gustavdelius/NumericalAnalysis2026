...
::: {#exr-5.32}
💻 💬 The midpoint method only uses the slope at the midpoint. Perhaps one can find a better method by taking some combination of the slopes at the start point, the midpoint and the end point as follows:
...
-   Now we have three estimates of the slope and could take a weighted average of them:
    $$
    \text{estimate of slope} = \frac{c_n m_n + c_{n+1/2} m_{n+1/2} + c_{n+1} m_{n+1}}{c_n + c_{n+1/2} + c_{n+1}} 
    $$

    where $c_n$, $c_{n+1/2}$ and $c_{n+1}$ are constants that you get to choose. We will then take a step forwards using this estimate of the slope:
...
plt.show()
```
Collaborate within your group to extend the table below.


|     | $c_n$ | $c_{n+1/2}$ | $c_{n+1}$ | **Order of Error**          | **Name**        |
|-----|---------|---------------|---------|-----------------------------|-----------------|
| 1   | 1       | 0             | 0       | $\mathcal{O}(\Delta t)$   | Euler's Method |
| 2   | 0       | 1             | 0       | $\mathcal{O}(\Delta t^2)$ | Midpoint Method |
|3    | 1/2     | 1/2           | 0       |                             |                 |
|4    | 1/6     | 2/3           | 1/6     |                             |                 |
|5    | 1/4     | 1/2           | 1/4     |                             |                 |
|6    |         |                 |         |                             |                 |
...
3.  Experiment with several ideas about how to best combine the four slopes: $m_n$, $m_{n+1/2}$, $m_{n+1/2}^*$, and $m_{n+1}^*$. Experiment with many different linear combinations
$$
...
The ratio $\gamma_{\max}/\gamma_{\min}$ of the fastest-decaying rate to the slowest-decaying rate is called the **stiffness ratio**. For our 1D example the stiffness ratio is simply 100 (the decay rate of the transient compared to the decay rate of... well, there is only one rate). The stability condition for the forward Euler method applied to $x'=\lambda x$ requires $h < 2/|\lambda|$, so a large $\lambda$ forces a tiny step size even if the interesting dynamics are slow.
...
x(t)=Ce^{-100t}+\frac{100}{10001}\sin t-\frac{1}{10001}\cos t
$$
is indeed the general solution to $x'=-100x+\sin t$. (Recall how we found the particular solution to $x'=-\frac{1}{3}x+\sin t$ in @exr-5.13.) Then:

a. What value of $C$ gives the solution satisfying the initial condition $x(0)=0$?
b. The transient $e^{-100t}$ has decayed to less than $1\%$ of its initial value by time $t^*$. Find $t^*$.
c. For $t>t^*$ the solution is well-approximated by $\frac{100}{10001}\sin t-\frac{1}{10001}\cos t$. Roughly how large is this "steady-state" amplitude?
d. The stability condition for the forward Euler method applied to $x'=\lambda x$ is $h<2/|\lambda|$. What is the maximum step size that forward Euler can use on this equation and still be stable?
:::
...
c. Repeat with the RK4 method. What is the largest step size for which RK4 remains stable on this problem? (Hint: the stability region of RK4 extends to roughly $|z|\lesssim 2.8$ along the negative real axis.) Compare with what you found for forward Euler.
...
$$
which describes a spring-mass system with damping. Writing $u=x$ and $v=x'$ converts this to the first-order system
$$
...
a. Write out the two scalar differential equations that make up this system.
b. Verify that $x(t)=C_1 e^{-t}+C_2 e^{-\gamma t}$ is the general solution to $x''+(\gamma+1)x'+\gamma x=0$ by substitution.
c. Use the initial conditions $u(0)=1$ and $v(0)=x'(0)=0$ to find $C_1$ and $C_2$.
d. For $\gamma=100$, how small must the step size $h$ be for the forward Euler method to be stable? How does this compare to the timescale on which the solution $u(t)=x(t)$ actually changes significantly?
:::

------------------------------------------------------------------------

::: {#exr-5.58}
💻 Solve the stiff system from @exr-5.57 numerically with $\gamma=100$ on the interval $t\in[0,5]$.

a. Using the result of @exr-5.57c, write down the exact solution $x(t)$ for these initial conditions and plot it.
b. Solve the system with the forward Euler method and step sizes $h=0.1$, $h=0.01$, and $h=0.001$. At which step size does forward Euler first produce a stable solution?
c. Solve the system with the backward Euler method using the same step sizes. Does backward Euler remain stable even for $h=0.1$?
d. 💬 By what factor can backward Euler take larger steps than forward Euler and still be stable? How does this factor relate to $\gamma$?
:::
