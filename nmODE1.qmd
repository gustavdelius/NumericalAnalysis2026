...
This chapter is still undergoing changes, to be released on Thursday of week 6.
...
ax.set_title("Euler", fontsize=22)
ax.set_xlim(0, 1.1)
ax.set_ylim(0.8, 3)
ax.grid(True)
...
ax.set_title("Midpoint", fontsize=22)
ax.set_xlim(0, 1.1)
ax.set_ylim(0.8, 3)
ax.grid(True)
ax.tick_params(labelsize=16)
...
![Graphical depictions of two numerical methods: Euler (left) and Midpoint (right). The exact solution is shown in red.](figures/ODE/Midpoint.png){#fig-5.4 alt="Graphical depictions of two numerical methods: Euler (left) and Midpoint (right). The Euler method uses the slope $m$ at the starting point whereas the midpoint method uses the slope aThe exact solution is shown in red."}
...
a.  Use your `euler1d` function to solve the same differential equation $x' = -3x$ with $x(0) = 1$ on the interval $t \in [0, 10]$. Run it for different values of the stepsize $\Delta t$, for example $\Delta t \in \{0.1, 0.5, 0.7, 1.0\}$.
...
d.  Try the same experiment using your `midpoint1d` function. Does the midpoint method also suffer from numerical instability? If so, what is the critical stepsize for the midpoint method on this problem?
:::
