...
You can also do this directly in Python: simply evaluate the function at many points and choose the smallest value: 
...
::: {#exr-3.57}
💬 What is the Gradient Ascent/Descent algorithm doing geometrically? Draw a picture and be prepared to explain to your peers. Discuss what would be a good stopping rule. The conventional stopping rule is to stop when $|f'(x_n)|$ is smaller than a chosen tolerance. How is this different from the stopping rule you used in Newton's method?
:::

------------------------------------------------------------------------

::: {#exr-3.53}
💻 Write code to implement the 1D gradient descent algorithm. Note that gradient descent is a fixed point iteration method So you can reuse the fixed point iteration code from @exr-fp_python or adapt the code for Newton's method from @exr-2.38. Just be sure to change the iteration formula and the stopping criterion. 

``` python
def gradient_descent(df, x0, alpha, 
...
```python
def gradient_descent_2d(df, x0, alpha, tol=1e-12, max_iter=10000):
    """
    ...
    df (function): A function that returns the gradient of the function to
                   minimise. It should take a NumPy array with two elements
...
1.  Implement the code above then spend some time playing around with the `minimize` command to minimise more challenging functions.
...
In this section we will change our focus a bit to look at a different question from algebra, and, in turn, reveal a hidden numerical optimisation problem where the `scipy.optimise.minimize()` tool will come in quite handy.
...
You may recognize this as the basic question of regression from statistics. What we will do here is pose the statistical question of which curve best fits a data set as an optimisation problem. Then we will use the tools that we have built so far to solve the optimisation problem.
