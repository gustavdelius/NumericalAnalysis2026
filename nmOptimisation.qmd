...
4. 🎓 Add a print statement to you `plot_error_progression()` function that prints out the slope of the straight line fitted to the points, similar to how you did that in @exr-2.46.
...
The following figure allows you to look at this function from all directions. Just drag it around with your mouse.
```{python}
#| label: fig-opt-3
...
$$
|\boldsymbol{x}_{n+1}-\boldsymbol{x}_{n+1}|=\sqrt{\sum_{i=0}^{k-1}(x_{n+1,i}-x_{n,i})^2} < \text{tol}
$$
