...
$$
 You know the values of $h=\Delta t$, $t_{n+1}$ and $x_n$, and you know the function $f$, so, in a practical sense, you should use some sort of numerical method to solve that equation -- at each time step. Given that we do not know the derivative of $G$, we can not use Newton's method. So our next best option is to use the secan method. You can use your code from @exr-2.49 for the function `secant_1d()`.
...
(b) Show that the backward Euler method is unconditionally stable for the linear differential equation $x' = -\lambda x$ where $\lambda > 0$.

(c) Show that the backward Euler method has a local truncation error of order $O(h)$.

(d) State, but don't derive, the order of accuracy for the Runge-Kutta 4 (RK4) method? Assuming you are using RK4 to approximate the solution to $x'=f(t,x)$, how many evaluations of the function $f(t,x)$ are required at each time step? [2 marks]
