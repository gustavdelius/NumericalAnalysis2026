...
$$
 u(0,x) = \sin(2 \pi x), \, u(t,0) = 0, \, \text{and} \, u(t,1) = 0.
$$
...
::: {#exr-6.31a}
💻 💬 Now run your `heat1d` function from @exr-heat1d with the same diffusion coefficient $D=0.1$, the same step sizes $\Delta t=0.01$ and $\Delta x=0.01$, and the same initial and boundary conditions but run it for a longer time $t=0.5$ and plot the solution on the domain $t\in[0,0.5]$ and $x\in[0,1]$. Do you believe what you see? What is happening to the solution?
:::
...
$$
\mathbf{\epsilon}_n=\sum_i\epsilon_i\lambda_i^n\mathbf{v}_i.
$$
...
$$
0\leq a\left(\sin\left(\frac{k\pi}{2N}\right)\right)^2<\frac12
$$
