...
c. For $t>t^*$ the solution is well-approximated by the oscillatory function $\frac{100}{10001}\sin t-\frac{1}{10001}\cos t$. How large is the amplitude (the largest value) of this oscillation?
d. What is the maximum step size that forward Euler can use on this equation and still be stable?
:::
...
The ratio $\gamma_{\text{fast}}/\gamma_{\text{slow}}$ of the fastest-decaying rate to the slowest-decaying rate is called the **stiffness ratio**. It describes how much smaller the step size must be for an explicit method to remain stable compared to the step size required for an implicit method.
...
a. Solve the system with the forward Euler method and step sizes $h=0.1$, $h=0.01$, and $h=0.001$. At which step size does forward Euler first produce a stable solution?

b. Solve the system with the backward Euler method using the same step sizes. Does backward Euler remain stable even for $h=0.1$?
