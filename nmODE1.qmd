...
Her we consider the initial-value problem $x'(t) = f(t, x(t))$ with exact solution $x(t)$. The **local truncation error** at step $n$ is defined as the error made in a single step starting from the exact solution, divided by the step size $h$:
$$
