...
*   (Alternatively, using the area of a single trapezoid or a geometric argument directly: $\frac{f(0)+f(5)}{2} \times 5 = \frac{3 + 28}{2} \times 5 = 15.5 \times 5 = 77.5$).
...
**(d) Python Code for `odeint`**

``` python
# F should be a function of (x, t, g, L) where x is an array [theta, omega]
F = lambda x, t, g, L: [ x[1], -(g/L)*np.sin(x[0]) ]

x = scipy.integrate.odeint(F, x0, t, args=(g, L))
